...
//  Edited my Andrew Alagna
//  Added  invert, invertRest, and rotate
...
void LinkedList<T>::invert() {
    ...
    if(item_count_ > 1) //if the list has items, invert it
...
void LinkedList<T>::invertRest(Node<T> *current_first_ptr) {
    if (current_first_ptr->getNext() == nullptr)
        ...
        invertRest(current_first_ptr->getNext());//Recursive call to function
...
void LinkedList<T>::rotate(int k) {
    ...
    if (k > 0) {
        insert(0, getEntry(item_count_ - 1));
        remove(item_count_ - 1);
        rotate(k - 1); //Recursive fall to rotate
    }
