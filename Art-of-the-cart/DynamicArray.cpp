...
bool DynamicArray<T>::remove(const T &an_item)
{
    try
    {
        bool canRemoveItem = !isEmpty() && (contains(an_item)); //**taken out of textbook from pg. 110.**
        if (canRemoveItem) {
            for (int index = getIndexOf(an_item); index < item_count_; index++) {
                items_[index] = items_[index + 1];//remove given item by copying each item in array to the item in front of it.
            }
            item_count_--;
...
void DynamicArray<T>::resize()
{
    ...
    if(atCapacity()) {
        T* new_array = new T[current_capacity_ * 2];
        for (int i = 0; i < current_capacity_; i++) {//copy the old array into new dynamic array if array contains items.
            new_array[i] = items_[i];
        }
        setItems(new_array, item_count_);
        current_capacity_ = item_count_ * 2;

    }else if (item_count_ < (current_capacity_/4)) {
        T* new_array = new T[current_capacity_ / 2];
        for(int i = 0; i < item_count_; i++) {//copy the old array into new dynamic array if array contains items.
            new_array[i] = items_[i];
        }
        setItems(new_array, item_count_);
        current_capacity_ = (item_count_ * 2) + 2;
    }
